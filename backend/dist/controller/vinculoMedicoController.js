...
const vinculoMedService_1 = __importDefault(require("../service/vinculoMedService")); // Importa a instância
const ExcelService_1 = __importDefault(require("../service/ExcelService")); // Importa a instância
class VinculoMedicoController {
    vinculoMedicoService;
    excelService;
    constructor() {
        this.vinculoMedicoService = new vinculoMedService_1.default();
    ...
    vinculaMedico = async (req, res, next) => {
        try {
            ...
            const result = await this.vinculoMedicoService.realizaVinculoMedico(crm, uf, cpf);
            if (result.success) {
                res
                    .status(201)
    ...
    buscaMedico = async (req, res, next) => {
        try {
            ...
            const result = await this.vinculoMedicoService.buscaMedico(crmValue, ufValue);
            if (result.success) {
    ...
    vinculaMedicoBatch = async (req, res, next) => {
        try {
            if (!req.file || !req.file.buffer) {
                throw new appError_1.AppError("Nenhum ficheiro Excel foi enviado ou o ficheiro está vazio.", 400);
            }
            ...
            const excelProcessResult = await this.excelService.processVinculoExcel(req.file.buffer, requiredHeaders);
            ...
            if (excelProcessResult.data.length === 0) {
                throw new appError_1.AppError(excelProcessResult.message ||
                    "Nenhuma linha válida encontrada na planilha para processamento.", 400 // Bad Request, pois o ficheiro não continha dados processáveis
                );
            }
            ...
            const batchProcessResult = await this.vinculoMedicoService.processaVinculoMedicoBatch(excelProcessResult.data);
            ...
            if (batchProcessResult.successfulUpdates > 0) {
                finalMessageParts.push(`${batchProcessResult.successfulUpdates} vínculos atualizados com sucesso.`);
            }
            if (batchProcessResult.failedUpdates > 0) {
                finalMessageParts.push(`${batchProcessResult.failedUpdates} vínculos falharam na atualização.`);
                overallSuccess = false; // Se houve falhas no DB/negócio, o sucesso geral é parcial ou falho
            }
            if (excelProcessResult.failedCount > 0) {
                finalMessageParts.push(`${excelProcessResult.failedCount} linhas foram ignoradas devido a erros estruturais.`);
            ...
            const finalMessage = finalMessageParts.length > 0
